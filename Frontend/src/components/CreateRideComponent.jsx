...
export default function CreateRideComponent() {
  ...
  return (
    <form className="rounded-lg bg-white shadow-md" onSubmit={formPublishHandler}>
      <PublishRideInputs />
      <div>
        <button
          className="bg-green-500 w-full py-3 px-4 rounded-b-lg text-xl hover:bg-green-600 font-semibold"
          type="submit"
          onClick={publishRideHandler}
        >
          Publish ride
        </button>
      </div>
...
function TimePicker({ title, atomName, minVal }) {
  ...
  return (
    <div className="grow my-2">
      <label htmlFor={title.split(' ')[0]} className="text-slate-600">
        {title}
      </label>
      <div>
        <div className="w- relative flex">
          <input
            ...
            value={atomGetter}
            onChange={(e) => {
              atomSetter(e.target.value);
            }}
            id={title.split(' ')[0]}
            className="w-full text-lg hover:bg-slate-100 mt-2 p-1 border-transparent rounded-md focus:outline-none focus:border-transparent focus:ring-0"
            required
            {...(minVal ? { min: minVal } : {})}
          />
        </div>
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        name="fare"
        id="fare"
        className="pl-[28px] py-2 p-1 w-full text-md hover:bg-slate-100 font-semibold border-transparent rounded-md focus:outline-none focus:border-transparent focus:ring-0"
        min="0"
        max="10000"
        value={atomGetter}
