...
function UpcomingRides() {
  ...
  const findPassengerRequests = async () => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/rides/fetchUpcomingRides/?email=${user.email}`
      );
  ...
  const markCompleted = async (curr) => {
    try {
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/rides/markCompleted`, { curr });
  ...
  return (
    <div>
      {upcomingRides.length === 0 ? (
        <div className='w-full h-screen flex justify-center items-center font-bold text-xl'>
          ...
          {upcomingRides.map((ride, i) => (
            <div key={i}>
              <div className='w-full bg-slate-200 flex flex-col lg:flex-row justify-between items-center px-10 py-2 my-2'>
                ...
                <h1 className='flex-1 w-full text-center font-bold'>
                  {ride.ride.departureTime.split('T')[0]}
                </h1>
                {user.email !== ride.rider.email && (
                  <button
                    className='flex-1 w-full py-2 my-4 rounded-lg text-white bg-green-400 font-bold'
                    onClick={() => {
                      markCompleted(ride);
                    }}
                  >
